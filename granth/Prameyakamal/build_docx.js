...
const OUT_FILE = process.env.OUT || path.resolve(WORK, META.out || path.join('..', 'Vyakhya.docx'));
...
// **bold**, *italic* (single asterisks) and `code` (ticks dropped); an unpaired * stays literal.
function parseInline(line) {
  line = line.replace(/`([^`]+)`/g, '$1');  // inline-code ticks carry no meaning in print
...
function parseAddendum(num) {
  // addenda may be named s07.md, s007.md, s0007.md or s7.md — accept any zero-padding
  const f = [4, 3, 2, 1].map(w => path.join(ADD_DIR, `s${String(num).padStart(w, '0')}.md`)).find(fs.existsSync);
  if (!f) return null;
...
const kind = h => {
  ...
  if (h.includes('अनुवाद')) return 2;
  if (h.includes('पूरक')) return 3.5;
  if (h.includes('व्याख्या') || h.includes('विश्लेषण')) return 3;
  if (h.includes('उदाहरण')) return 4;
  if (h.includes('तुलनात्मक') || h.includes('तालिका') || h.includes('सारणी') || h.includes('वर्गीकरण')) return 5;
  if (h.includes('सन्दर्भ') || h.includes('टिप्पणी')) return 6;
  if (h.includes('सार') || h.includes('नोट')) return 3.5;
  return 0;
};
