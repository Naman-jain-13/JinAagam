...
FORBIDDEN = [
    ...
    "बैच", "batch", "अगले बैच", "अगले सत्र", "progress.md", "हमने पिछली बातचीत",
    # "बैठक" (a work session) removed for this ग्रन्थ: it is a substring of the ordinary Hindi verb
    # "बैठकर" ("having sat"), which occurs innocently 13 times — "जिसमें बैठकर जीव…", "गुफामें
    # बैठकर…", and in the निर्वचन 'जहाँ बैठकर देखे'. A check for the standalone word found none.
...
def check_docx(path):
    ...
    try:
        ...
        for p in doc.paragraphs:
            st = sname(p)
            if st == "Heading 3":
                # Keep this in step with postprocess_docx.py: "mool" means quoted text that must not be
                # touched, and in this ग्रन्थ that is parts 1–4 — the मूल गाथा, its संस्कृत छाया, the अन्वय,
                # and the अन्वयार्थ (whose पद, Sanskrit forms and compound splits quote the verse).
                # Without this the checker reports 'देवसेनदेवम्' inside a grammatical parenthesis as a
                # bare आचार्य name that postprocess failed to honour, which is exactly what it should be.
                quoted = "मूल" in p.text or "छाया" in p.text or "अन्वय" in p.text
                sec = "mool" if quoted else ("ref" if "सन्दर्भ" in p.text and "टिप्पणी" in p.text else "other")
                continue
