...
for i in range(0, len(parichhed_blocks), 2):
    ...
    for idx, (topic, page_start) in enumerate(rows):
        ...
        label = topic
        if idx == 0:  # हर परिच्छेद की पहली पंक्ति पर "तृतीय परिच्छेद (परोक्ष) — …"
            ordinal, _, pname = parichhed_name.partition(" — ")
            label = f"{ordinal} ({pname}) — {topic}" if pname else f"{ordinal} — {topic}"
        key = str(num)
        if key in groups:
