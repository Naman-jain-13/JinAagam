// v2 builder: Bruhat Sarvagya Siddhi vyakhya (English numerals outside mool text) -> docx in the Pramana_Pariksha_Sampurna_Vyakhya format.
//
// Inputs
...
//
// Output: ./vyakhya/Bruhat_Sarvagya_Siddhi_Vyakhya.docx  (TOC + PAGE fields; run finalize_word.ps1 to update fields & export PDF)
...
const OUT_FILE = process.env.OUT || path.join(__dirname, 'vyakhya', 'Bruhat_Sarvagya_Siddhi_Vyakhya.docx');

const DEV = '०१२३४५६७८९';
const devToInt = s => parseInt(s.replace(/[०-९]/g, d => DEV.indexOf(d)), 10);
const intToDev = n => String(n).replace(/\d/g, d => DEV[d]);
const toEn = t => t.replace(/[०-९]/g, d => String(DEV.indexOf(d)));

// ---------- inline / paragraph helpers ----------
function parseInline(line) {
  // **bold**, *italic* (single asterisks, not adjacent to another asterisk)
  const runs = []; const re = /\*\*(.+?)\*\*|(?<![*\w])\*(?!\s)([^*]+?)(?<!\s)\*(?![*\w])/g; let last = 0, m;
  while ((m = re.exec(line)) !== null) {
    if (m.index > last) runs.push({ text: line.slice(last, m.index), bold: false });
    if (m[1] !== undefined) runs.push({ text: m[1], bold: true }); else runs.push({ text: m[2], bold: false, italics: true });
    last = re.lastIndex;
  }
...
const runs = (line, extra) => { const keep = extra && extra.color === '7A1F1F'; return parseInline(line).map(r => new TextRun(Object.assign({ text: keep ? r.text : toEn(r.text), bold: r.bold, italics: !!r.italics, size: BODY, font: FONT }, extra || {}))); };
...
function buildTable(rows) {
  ...
  const trs = rows.map((cells, ri) => new TableRow({
    ...
    children: Array.from({ length: n }, (_, ci) => new TableCell({
      ...
      children: [new Paragraph({ spacing: { after: 20, line: 280 }, children: parseInline(cells[ci] || '').map(r => new TextRun({ text: toEn(r.text), bold: ri === 0 || r.bold, italics: !!r.italics, size: BODY - 3, font: FONT })) })]
    }))
  }));
...
const kind = h => {
  ...
  if (h.includes('सारणी') || h.includes('तालिका')) return 5;
  if (h.includes('खण्ड-सार') || h.includes('नोट') || h.includes('सार-व्याख्या') || h.includes('विषय-सार')) return 3.5;
...
function renderSection(s) {
  ...
  byKind(2).forEach(x => out.push(...pg(x), ...renderBlock(x.lines)));
...
const front = [
  ...
  new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 240 }, children: [new TextRun({ text: 'बृहत्-सर्वज्ञ-सिद्धिः', bold: true, size: 48, font: FONT, color: '7A1F1F' })] }),
  new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 200 }, children: [new TextRun({ text: `सम्पूर्ण ग्रन्थ (§1 – §${secCount}): अनुच्छेदशः मूल संस्कृत पाठ, हिन्दी अनुवाद, जैनागम-सम्मत विस्तृत व्याख्या, सरल उदाहरण, तुलनात्मक तालिका/चार्ट एवं सन्दर्भ`, size: BODY, font: FONT })] }),
  pNormal('**आधार:** उपलब्ध 75-पृष्ठीय स्कैन — "बृहत् सर्वज्ञ सिद्धि [आचार्य अनन्तकीर्ति विरचित], सम्पादक पं. कल्लप्पा निटवे" (मुद्रित पृष्ठ 130–204; उसी संग्रह-ग्रन्थ का अंश जिसमें लघु-सर्वज्ञ-सिद्धि है)। मूल पाठ स्कैन-छवियों को पढ़कर लिपिबद्ध किया गया है; जहाँ अक्षर अस्पष्ट थे वहाँ [अस्पष्ट] अंकित है। स्कैन पुष्पिका "इति सर्वज्ञसिद्धिः कृतिर्भट्टानन्तकीर्तेः… समाप्तोऽयं ग्रन्थः" के साथ पूर्ण है। इस व्याख्या में स्कैन-पृष्ठ (1–75) का उल्लेख है; मूल पाठ में श्लोक-संख्याएँ देवनागरी अंकों में यथावत् रखी गई हैं, शेष सर्वत्र अंग्रेज़ी अंक प्रयुक्त हैं।'),
  ...
  pNormal('**ग्रन्थ का विषय:** यह कृति लघु-सर्वज्ञ-सिद्धि का विस्तृत रूप है। मीमांसक, बौद्ध और लौकिक पूर्वपक्ष की सर्वज्ञ-विरोधी युक्तियों (13 पृष्ठ) को पूरा सुनकर यह तीन बड़े खण्डों में उत्तर देती है — (1) "असम्भव-अर्थ" और अभाव-प्रमाण-सम्बन्धी आपत्तियों का खण्डन, (2) हेत्वाभास-निराकरण और ग्रहण-उपदेश से धर्मज्ञता-सर्वज्ञता तक विस्तार, (3) मोक्ष-मार्ग (रत्नत्रय) का साक्षात्कार, बौद्ध नैरात्म्यवाद का खण्डन, सप्त-तत्त्व की आवश्यकता और संचित-कर्म-क्षय की मीमांसा — और अन्त में सिद्ध करती है कि असर्वज्ञ भी सर्वज्ञ को पहचान सकता है, पर सर्वज्ञ का अभाव किसी प्रमाण से नहीं जान सकता। अनुच्छेद-विभाजन (§) एवं शीर्षक व्याख्याकार के हैं, मूल ग्रन्थ के नहीं।'),
...
const doc = new Document({
  creator: 'JinAagam',
  title: 'बृहत्-सर्वज्ञ-सिद्धिः — सम्पूर्ण व्याख्या',
